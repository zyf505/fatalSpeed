...
public class fatalSpeed extends PApplet {
...
public void setup() {
  ...
  e=PApplet.parseInt(random(0,9000));
  x7=random(100, 620);
  y8=-1000;
}
public void draw() {
  if (stage==0) {  //welcome page
    ...
    if (time%300==0&&time>=300) {// generate new car blocks every 5 seconds
      for (int i=0; i<PApplet.parseInt (random (0, 3)); i++) { 
    ...
    if (time%1800==0&&time>0) {
      d=PApplet.parseInt(random(0, 360));
    ...
    if (pause==false) {
      spe=spe+0.00185f;  //speed increases to 15(300km/h) in 90s
      if (spe>15) {
        spe=15;
      }
...
public void notice() {// draw notice sign at top right corner
  fill(255, 0, 0);
  textSize(30);
  text("!!!!", 680, 40);
}
public void stop() {//necessary function for music
  player1.close();
  player2.close();
  player3.close();
  minim.stop();
}
public void parameter() {//display fuel, speed, score and life at top lift corner
  fill(250, 0, 0);
  textSize(30);
  if (pause==false) {
    text("Speed:"+PApplet.parseInt(spe*20)+"km/h", 10, 40);
  } else {
    text("Speed:"+PApplet.parseInt(spe1*20)+"km/h", 10, 40);
  }
  text("Fuel:"+PApplet.parseInt((9000-time)/900)+"L", 10, 80);
  text("Score:"+score, 10, 120);
  text("Life:"+life, 10, 900);
  text("Laser:"+lasernum, 10, 940);
}
public void control() {// key control
  if (keyPressed==true) { 
...
public void enlarge(int a) {// enlarge the buttons 
  w[a]=180;
  h[a]=72;
}
public void shrink() {// shrink to original size
  for (int i=0; i<5; i++) {
    w[i]=150;
    h[i]=60;
  }
...
public void flash() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "fatalSpeed" };
    if (passedArgs != null) {
